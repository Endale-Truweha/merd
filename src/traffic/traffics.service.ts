...
//console.log(smallerArrays)
   ...
   */
   private async fetchSensorHistoricData(sensorId: number, startDate: string, endDate: string): Promise<any> {
    console.log(sensorId)
    const apiUrl = `http://172.16.10.138:8080/api/historicdata.json?id=${sensorId}&avg=3600&sdate=${startDate}&edate=${endDate}&usecaption=1&username=Yonas.M&password=Test@123`;

    return axios
      .get(apiUrl)
      .then(response => ({ sensorId, data: response.data }))
      .catch(error => {
        console.error(`Error fetching data for sensor ID ${sensorId}:`, error);
        return null;
      });
  }
...
for (const sensorChunk of smallerArrays) {
    ...
    console.log(startDate,endDate)
    const requests = sensorChunk.map(async sensorId =>
    await  this.fetchSensorHistoricData(sensorId, startDate, endDate)
